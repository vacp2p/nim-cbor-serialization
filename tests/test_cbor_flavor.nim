...
proc writeValue*(
    w: var CborWriter[StringyCbor], val: SomeInteger) {.raises: [IOError].} =
  writeValue(w, $val)
...
#  }
#}
const cborText = "0xA4636F6E65737468697320746578742077696C6C20676F6E656374776F3903146574687265651903E764666F7572A2656170706C658301F56574687265656662616E616E61A36463686970187B617AF66176F4"
...
suite "Test CborFlavor":
  test "basic test":
    ...
    check encoded.hex == "0xa4646e616d6561636178632d31306179623230646c6973748361316132623235"
  ...
  test "Write special types":
    let vv = Cbor.decode(cborText.fromHex, SpecialTypes)
    ...
    check:
      ...
      xx.hex == "0xA36374776F3903146574687265651903E764666F7572A2656170706C658301F56574687265656662616E616E61A36463686970187B617AF66176F4".toLowerAscii

  test "object with null fields":
    expect CborReaderError:
      let x = Cbor.decode(cborTextWithNullFields.fromHex, Container)
      discard x

    let x = NullyFields.decode(cborTextWithNullFields.fromHex, Container)
    check x.list.len == 0

    # field should not processed at all
    let y = NullyFields.decode(cborTextWithNullFields.fromHex, ListOnly)
    check y.list.len == 0
