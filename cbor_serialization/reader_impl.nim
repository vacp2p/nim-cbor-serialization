...
proc read*[T: object](
  ...
  mixin cborAllowsUnknownFields, cborRequiresAllFields
  type
    ReaderType = typeof(r)
  ...
  when fieldsTable.len > 0:
    ...
    r.parseObject(key):
      ...
      elif cborAllowsUnknownFields(Flavor):
        r.skipSingleValue()
      else:
        r.parser.raiseUnexpectedField(key, cstring typeName)

    if cborRequiresAllFields(Flavor) and
    ...
    r.parseObject(key):
      ...
      if cborAllowsUnknownFields(Flavor):
        r.skipSingleValue()
      else:
        r.parser.raiseUnexpectedField(key, cstring typeName)
...
proc read*[T: tuple](
  ...
  r.parseArray:
    ...
    inc i
  if cborRequiresAllFields(Flavor) and i < numFields:
    r.parser.raiseIncompleteObject(typeName)
