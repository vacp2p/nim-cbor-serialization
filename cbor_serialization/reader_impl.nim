...
proc readRecordValue*[T](r: var CborReader, value: var T)
...
template readValueObjectOrTuple(Flavor, r, value) =
  ...
  when not isAutomatic:
    const
      flavor =
        "CborReader[" & typetraits.name(typeof(r).Flavor) & "], " &
        typetraits.name(T)
    {.error:
      "Missing Cbor serialization import or implementation for readValue(" &
      flavor & ")".}
...
proc parseStringEnum[T](
  ...
  try:
    ...
    raiseUnexpectedValue(r.parser, "Invalid value for '" & typeName & "'")

func strictNormalize(s: string): string =  # Match enum value exactly
  s

proc parseEnum[T](
    r: var CborReader, value: var T, allowNumericRepr: static[bool] = false,
  ...
  of CborValueKind.Number:
    when allowNumericRepr:
      ...
      of EnumStyle.Numeric:
        if not value.checkedEnumAssign(r.parseInt(int)):
          const typeName = typetraits.name(T)
          raiseUnexpectedValue(r.parser, "Out of range for '" & typeName & "'")
      of EnumStyle.AssociatedStrings:
        raiseUnexpectedValue(r.parser, "string", $r.parser.cborKind())
    else:
      raiseUnexpectedValue(r.parser, "string", $r.parser.cborKind())
  else:
    case style
    of EnumStyle.Numeric:
      when allowNumericRepr:
        raiseUnexpectedValue(r.parser, "number or string", $r.parser.cborKind())
      else:
        raiseUnexpectedValue(r.parser, "number", $r.parser.cborKind())
    of EnumStyle.AssociatedStrings:
        raiseUnexpectedValue(r.parser, "string", $r.parser.cborKind())

iterator readArray*(r: var CborReader, ElemType: typedesc): ElemType
...
iterator readObjectFields*(r: var CborReader, KeyType: type): KeyType
    {.raises: [IOError, SerializationError].} =
  mixin readValue
...
iterator readObject*(r: var CborReader, KeyType: type, ValueType: type): (KeyType, ValueType)
    {.raises: [IOError, SerializationError].} =
  mixin readValue
...
proc readValue*[T](r: var CborReader, value: var T) {.raises: [SerializationError, IOError].} =
  mixin readValue
  ...
  when value is CborValueRef:
    ...
    autoSerializeCheck(Flavor, array, typeof(value)):
      ...
      if val.len != value.len:
        raiseUnexpectedValue(r.parser, "string of wrong length")
    ...
    autoSerializeCheck(Flavor, array, typeof(value)):
      ...
      r.parseArray(idx):
        if idx < value.len:
          ...
          raiseUnexpectedValue(r.parser, "Too many items for " & $(typeof(value)))
    ...
    {.error: flavorName & ": Failed to convert from CBOR an unsupported type: " &
      typeName.}
