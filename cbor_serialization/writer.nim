...
export
  outputs, format, types, DefaultFlavor
...
proc writeMember*[V: not void](w: var CborWriter, name: string, value: V) {.raises: [IOError].}
...
proc writeHead(w: var CborWriter, majorType: uint8, argument: uint64)
    {.raises: [IOError].} =
  let minor = argument.toMinorLen()
  w.stream.write initialByte(majorType, minor)
  case minor
  of minorLen1: w.stream.write argument.uint8
  of minorLen2: w.stream.write argument.uint16.toBytesBE()
  of minorLen4: w.stream.write argument.uint32.toBytesBE()
  of minorLen8: w.stream.write argument.toBytesBE()
  else: discard
...
proc writeMember*[V: not void](
    w: var CborWriter, name: string, value: V) {.raises: [IOError].} =
  ## Write a member field of an object, i.e., the name followed by the value.
  ##
...
iterator stepwiseArrayCreation*[C](w: var CborWriter, collection: C): auto {.raises: [IOError].} =
...
template writeObjectField*[FieldType, RecordType](w: var CborWriter,
                                                  record: RecordType,
...
template beginRecord*(w: var CborWriter) = beginObject(w)
  ## Alias for beginObject, for record serialization.

template beginRecord*(w: var CborWriter, T: type) = beginObject(w, T)
  ## Alias for beginObject with type, for record serialization.

template writeFieldName*(w: var CborWriter, name: string) = writeName(w, name)
...
template endRecord*(w: var CborWriter) = w.endObject()
